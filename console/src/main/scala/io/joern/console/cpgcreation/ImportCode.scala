...
class ImportCode[T <: Project](console: io.joern.console.Console[T])(implicit
  ...
  /** Checks if the input path is valid, i.e., it exists and is not a directory. */
  private def checkInputPath(inputPath: String): Unit = {
    ...
    if (file.isDirectory) throw new ConsoleException(s"Input path is a directory: '$inputPath'")
  }

  /** Main method to import code and generate the CPG, deciding the frontend by language or content of input path */
  ...
  // Apply the generator to create CPG and apply overlays
